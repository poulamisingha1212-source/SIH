...
def resolve_house_selection(spec: str | None = None) -> list[str]:
...
def _clean_mp_name(name) -> str | None:
    if name is None:
        return None
...
def _short_work_type(activity_name) -> str | None:
...
def _work_id(rec: dict) -> str | None:
    try:
...
def datasets_to_long_frame(datasets: dict[str, list[dict]], house: str | None = None) -> pd.DataFrame:
...
class MPLADSLiveClient:
    ...
    def fetch_long_dataframe(self, houses: str | None = None) -> pd.DataFrame:
...
def fetch_live_long_dataframe(houses: str | None = None,
                              base_url: str = None,
