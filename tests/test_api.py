...
def test_human_review_workflow_and_rbac():
    ...
    from backend.auth import ROLE_MOSPI_REVIEWER, ROLE_PUBLIC_TIER, create_access_token
    list_res = client.get("/api/works?page_size=1")
    work_id = list_res.json()["items"][0]["work_id"]

    # 1. MoSPI Reviewer succeeds with valid JWT Bearer token
    mospi_token = create_access_token("Audit Officer Sharma", ROLE_MOSPI_REVIEWER)
    review_payload = {
    ...
    # 2. Public Tier token is forbidden
...
def test_unauthenticated_review_is_forbidden():
    """Fail-closed RBAC: an unauthenticated request must be rejected with 401."""
    list_res = client.get("/api/works?page_size=1")
    work_id = list_res.json()["items"][0]["work_id"]
    res = client.post(
        f"/api/works/{work_id}/review",
...
def test_sync_run_rejects_invalid_mode():
    from backend.auth import ROLE_MOSPI_REVIEWER, create_access_token
    mospi_token = create_access_token("admin_mospi", ROLE_MOSPI_REVIEWER)
    res = client.post(
