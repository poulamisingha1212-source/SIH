...
from backend.database import mp_allocations, review_logs, works
...
def _group_works(group_key: str | None, with_extremes: bool = False,
...
def _allocations_by_mp(house: str | None = None) -> dict:
...
def apply_house(filt: dict, house: str | None) -> dict:
...
def get_mp_profile(db, mp_name: str, house: str | None = None) -> dict | None:
...
def get_state_profile(db, state: str) -> dict | None:
...
def get_category_analytics(db, house: str | None = None) -> list:
...
def get_status_analytics(db, house: str | None = None) -> list:
...
def top_entity_stats(group_field: str, house: str | None = None,
                     limit: int = 8) -> list:
