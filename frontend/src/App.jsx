...
import { apiFetch, getAuthSession, clearAuthSession } from '@/lib/api';

export default function App() {
  const [activeTab, setActiveTab] = useState('overview'); // Default tab set to 'overview' (Dashboard)!
  
  // Initialize auth from persistent session if available
  const initialAuth = getAuthSession();
  const [currentRole, setCurrentRole] = useState(
    initialAuth.user?.role || 'Read-Only Public Tier'
  );
    ...
    try {
      const res = await apiFetch(`/api/works/${encodeURIComponent(workId)}/review`, {
        ...
        body: JSON.stringify({
          outcome,
          notes,
          reviewer_name: loggedInUser || (currentRole === 'MoSPI Reviewer' ? 'MoSPI Senior Auditor' : 'District Review Officer'),
