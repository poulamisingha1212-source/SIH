...
import { Lock, User, KeyRound, ShieldAlert, X, ShieldCheck } from 'lucide-react';
...
export default function LoginModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(data.detail || 'Wrong username or password');
        setIsSubmitting(false);
        return;
      }

      setAuthSession(data.access_token, { username: data.username, role: data.role });
      onSuccess(data.username, data.role, data.access_token);
    } catch (err) {
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
              Audit Portal Authentication
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Sign in with your authorized District Auditor or MoSPI Reviewer credentials
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                type="text"
                placeholder="Enter username (e.g. admin_mospi, district_auditor)"
                value={username}
